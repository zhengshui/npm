...
function addTmpTarball_ (tgz, data, shasum, cb) {
  ...
  getCacheStat(function (er, cs) {
    ...
    mkdir(pkg, function (er, created) {
      ...
      var write = fs.createWriteStream(targetTmp)
      var fin = cs.uid && cs.gid ? chown : done
      cb.track("%s -> %s", tgz, target)
  ...
  function done() {
    cb.track("done! size=%d", fs.statSync(target).size)
    data._shasum = data._shasum || shasum
    fs.rename(targetTmp, target, function (er) {
      cb(er, data)
    })
  }
}
