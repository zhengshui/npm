...
function findPeerInvalid (where, cb) {
  cb = cbTrack("findPeerInvalid "+where, cb)
...
function readDependencies (context, where, opts, cb) {
  cb = cbTrack("readDependencies ", cb)
  cb.track(where)
...
function installManyTop (what, where, context, cb_) {
  var cb = cbTrack("installManyTop " + where, function cb (er, d) {
...
function installMany (what, where, context, cb) {
  cb = cbTrack("installMany " + where, cb)
  ...
  readDependencies(context, where, opt, function (er, data, wrap) {
    ...
    asyncMap( what
      ...
      cb.track("resolved targets", targets.map(function (t) {
        return t._id
      }))
      ...
      asyncMap(targets, function (target, cb) {
        ...
        track("call installOne", target._id)
...
function installOne (target, where, context, cb) {
  cb = cbTrack("installOne " + target._id, cb)
  cb.track(where)
  ...
  after.track = cb.track
  installOne_(target, where, context, after)
  function after (er, installedWhat) {
  ...
  readJson(jsonFile, log.warn, function (er, data) {
    function thenLink () {
      npm.commands.link([target.name], function (er, d) {
        ...
        cb.track("localLink", "back from link", [er, d])
...
function installOne_ (target, where, context, cb) {
  cb.track("installOne_", target._id)
  var nm = path.resolve(where, "node_modules")
    , targetFolder = path.resolve(nm, target.name)
    , prettyWhere = path.relative(process.cwd(), where)
    , parent = context.parent
  ...
  chain
    ...
    , function (er, d) {
        var idx = installOnesInProgress[target.name].indexOf(where)
        if (idx !== -1) {
          installOnesInProgress[target.name].splice(idx, 1)
        }
