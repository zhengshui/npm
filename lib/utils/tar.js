...
var npm = require("../npm.js")
  ...
  , cbTrack = require("callback-tracker")
...
function unpack (tarball, unpackTarget, dMode, fMode, uid, gid, cb) {
  ...
  cb = cbTrack("unpack " + tarball, cb)
  cb.track(unpackTarget)
...
function gunzTarPerm (tarball, target, dMode, fMode, uid, gid, cb_) {
  ...
  cb.track("gunzTarPerm")

  if (gtpRunning > gtpParallel) {
    cb.track("running, queue for later")
    gtpQueue.push([tarball, target, dMode, fMode, uid, gid, cb])
    return
  }

  gunzTarPerm_(tarball, target, dMode, fMode, uid, gid, cb)
}

function gtpDequeue () {
  if (gtpRunning < gtpParallel) {
    var g = gtpQueue.shift()
    if (g) {
      gunzTarPerm_.apply(null, g)
    }
  }
}

function gunzTarPerm_ (tarball, target, dMode, fMode, uid, gid, cb) {
  gtpRunning ++

  cb.track("calling open")
  fs.open(tarball, 'r', function (er, fd) {
    cb.track("calling gtp__, fd=%j", fd)
    if (er) return cb(er)
    fs.fstat(fd, function (er, st) {
      if (er) return cb(er)
      if (st.size === 0) {
        er = new Error("zero-byte tgz: " + tarball + "\n" +
                       "Please run `npm cache clean`")
        return cb(er)
      }
      gunzTarPerm__(tarball, target, dMode, fMode, uid, gid, fd, st.size, cb)
    })
  })
}

function gunzTarPerm__ (tarball, target, dMode, fMode, uid, gid, fd, sz, cb) {
  var fst = fs.createReadStream(tarball, { fd: fd, start: 0, end: sz })


  var methods = ['destroy', 'close', 'open', 'resume', 'pause', 'push']

  methods.forEach(function (method) {
    fst[method] = function (original) { return function (a) {
      cb.track('fst.%s() arg=%s pos=%j end=%j',
               method, a, fst.pos, fst.end)
      return original.apply(this, arguments)
    }}(fst[method])
  })

  var readMethods = ['read', '_read']
  readMethods.forEach(function (read) {
    fst[read] = function (original) { return function () {
      cb.track('fst.%s()', read, fst.destroyed)
      fst[read] = original
  ...
  var evMethods = ['on', 'emit', 'once']
  evMethods.forEach(function (method) {
    fst[method] = function (original) { return function () {
      cb.track('fst.%s(%j)', method, arguments[0])
      return original.apply(this, arguments)
    }}(fst[method])
  })
  ...
  fst.on("error", function (er) {
    ...
    cb.track("fst error", er)
    cb(er)
  })

  cb.track("attach one-time data handler")
  fst.on("open", function (fd) {
    cb.track("open", fd)
  })

  fst.on("data", function OD (c) {
    cb.track("fst one-time data handler")
