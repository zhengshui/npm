...
function gunzTarPerm_ (tarball, target, dMode, fMode, uid, gid, cb) {
  ...
  fs.open(tarball, 'r', function (er, fd) {
    ...
    if (er) return cb(er)
    gunzTarPerm__(tarball, target, dMode, fMode, uid, gid, fd, cb)
  })
}

function gunzTarPerm__ (tarball, target, dMode, fMode, uid, gid, fd, cb) {
  var fst = fs.createReadStream(tarball, { fd: fd, start: 0 })
